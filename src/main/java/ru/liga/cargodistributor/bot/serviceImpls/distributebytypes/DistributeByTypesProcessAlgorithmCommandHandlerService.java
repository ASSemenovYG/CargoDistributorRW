...
@Service
public class DistributeByTypesProcessAlgorithmCommandHandlerService extends CommandHandlerService {
    private static final Logger LOGGER = LoggerFactory.getLogger(DistributeByTypesProcessAlgorithmCommandHandlerService.class);

    @Autowired
    protected DistributeByTypesProcessAlgorithmCommandHandlerService(@Value("${bot.token}") String token, @Value("${cache.capacity}") int cacheCapacity) {
        super(token, cacheCapacity);
    }

    public DistributeByTypesProcessAlgorithmCommandHandlerService(
