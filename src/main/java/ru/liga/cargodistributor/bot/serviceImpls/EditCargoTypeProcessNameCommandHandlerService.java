...
public class EditCargoTypeProcessNameCommandHandlerService extends CommandHandlerService {
    ...
    @Autowired
    protected EditCargoTypeProcessNameCommandHandlerService(@Value("${bot.token}") String token, @Value("${cache.capacity}") int cacheCapacity, CargoItemTypeRepository cargoItemTypeRepository) {
